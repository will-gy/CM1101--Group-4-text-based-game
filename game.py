...
def print_room(room):
   ...
   print_room_items(room)
                           #makes sure that guard is not generated along with the warden
...
e.g. execute_kill("shank")
"""

    global guard

    #loop through items in inventory looking for weapon id
    for item in inventory:
        if item["id"] == weapon:
            
            #when weapon get weapon damage
            damage = item["damage"]
